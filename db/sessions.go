...
func (db *DB) UpsertSession(s Session) error {
	query := `
		INSERT INTO sessions (
			...
			inbox_mode, auto_poll_interval, cron_job_id, last_active, registered_at, jsonl_path
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(session_id) DO UPDATE SET
			...
			cron_job_id = sessions.cron_job_id,
	...
	_, err := db.conn.Exec(query,
		...
		s.InboxMode, s.AutoPollInterval, s.CronJobID, s.LastActive, s.RegisteredAt, s.JSONLPath,
	)
	if err != nil {
...
func (db *DB) GetSession(sessionID string) (*Session, error) {
	...
	err := db.conn.QueryRow(query, sessionID).Scan(
		...
		&s.InboxMode, &s.AutoPollInterval, &s.CronJobID,
...
// ConfigureSession updates the inbox_mode, auto_poll_interval, and cron_job_id for a session.
func (db *DB) ConfigureSession(sessionID, inboxMode string, autoPollInterval *int, cronJobID *string) error {
	result, err := db.conn.Exec(
		"UPDATE sessions SET inbox_mode = ?, auto_poll_interval = ?, cron_job_id = COALESCE(?, cron_job_id) WHERE session_id = ?",
		inboxMode, autoPollInterval, cronJobID, sessionID,
	)
	if err != nil {
