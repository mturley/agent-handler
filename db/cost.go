...
func (db *DB) RecordCostTick(sessionID string, pid int, model, now, localDate string, reportedCost float64, reportedInput, reportedOutput int) error {
	...
	// nothing; the next climb is measured from the dipped value. A daily_cost
	// row means cost was actually attributed, so skip the upsert entirely on a
	// non-positive delta rather than writing a zero-cost row.
	if costDelta > 0 {
		if _, err := tx.Exec(`
