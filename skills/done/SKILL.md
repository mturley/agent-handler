...
Emit a status event summarizing what this session accomplished, so the handler knows the session ended intentionally (not mid-task).

## On invocation

1. Look back through the conversation for work completed since the last status event. Summarize it in 2-4 sentences covering: what was done, key decisions made, and any follow-up items.

2. Emit the status:
```bash
handler emit --type status --title "Session complete: <one-line summary>" --body "<detailed summary>"
```
...
2. Emit a status event indicating the session is resuming:
```bash
handler emit --type status --title "Session resuming: <brief description of new task>"
```
