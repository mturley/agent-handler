...
- `handler switch <session-name>` — switch cmux to another session's workspace and surface (requires cmux)
- `handler switch -a` — switch to the first session awaiting approval
...
```
handler switch <session-name-or-id>
handler switch --first-awaiting
```

Skills `/switch <name>` and `/awaiting` are shortcuts. When the user says "switch to that session" or "go to session X", use `handler switch`. If cmux is not available, tell the user switching requires cmux.
