...
var handlerCmuxActions = map[string]map[string]interface{}{
	"handler-switch-to-awaiting": {
		...
		"command":  "cmux reorder-surface --surface $CMUX_SURFACE_ID --index 9999; handler switch -a; exit",
	...
	"handler-switch-to-session": {
		...
		"command":  "cmux reorder-surface --surface $CMUX_SURFACE_ID --index 9999; handler switch; exit",
